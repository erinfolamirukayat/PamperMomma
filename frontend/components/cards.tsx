...
export function ServiceCard(props: Service) {
    ...
    const onClick = () => {
        router.push(`/mom/registries/${registryId}/services/${props.id}`);
    };

    const currentPercent = ((parseFloat(props.total_contributions) / (parseFloat(props.cost_per_hour) * props.hours)) * 100).toFixed(2)
    return (
        <article className={clx("flex flex-col gap-2 p-6 bg-white rounded-2xl shadow-md hover:shadow-lg transition-shadow cursor-pointer")} onClick={onClick}>
            <div className="flex flex-row gap-2 items-center mb-2">
                <div className="flex-1 bg-green-100 rounded-2xl overflow-hidden h-1">
                    <div className="h-full bg-green-500" style={{ width: `${currentPercent}%` }} />
                </div>
                <h5 className="text-label-desktop-small text-neutral-900">{currentPercent}%</h5>
            </div>

            <h3 className="text-title-desktop-small text-neutral-900">{props.name}</h3>
            <p className="flex-1 text-body-desktop-small text-neutral-700">{props.description}</p>
            <div className="flex flex-row justify-between mt-1 text-label-desktop-small">

                <span className="flex items-center gap-1">
                    {/* <Icon icon="material-symbols-light:schedule-outline" className="h-6 w-6 text-neutral-700" />
                    {props.totalHours} hrs */}
                </span>
                <span className="flex items-center gap-1">
                    <Icon icon="material-symbols-light:money-outline-rounded" className="h-6 w-6 text-neutral-700" />
                    ${props.total_contributions} out of ${props.total_cost} raised
                </span>
            </div>
            {/* {props.is_owned_by_user ?
                <ServiceOwnerModal
                    open={isOpen}
                    onClose={handleClose}
                    service={props}
